...
for i in range(1,len(masini)-1):
    ...
    masina_upper = masini[i].upper()
    print(masina_upper)
    masini[i] = masina_upper
else:
    print(masini)
...
for index in range(len(masini)):
    if masini[index] == 'Trabant' or masini[index] == 'Lăstun':
...
for masina,pret in pret_masini.items():
    if pret >=15000:
        break
    print(f'{masina} {pret} Pentru un buget de sub 15000 euro puteți alege mașină xLastun')
...
for numar in numere:
    if numar == 3:
        count += 1
print(f'Numarul 3 apare de:  {count}')
...
largest_number = numere[0]

for numar in numere:
    if numar > largest_number:
        largest_number = numar
